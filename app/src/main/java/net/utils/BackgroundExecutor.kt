...
object BackgroundExecutor {
    ...
    private fun directExecute(runnable: Runnable, delay: Long): Future<*>? {
        ...
        if (delay > 0) {
            ...
            val scheduledExecutorService = executor
    ...
    abstract class Task(id: String, delay: Long, serial: String) :
        Runnable {
        var id: String? = null
        var remainingDelay: Long = 0
        var targetTimeMillis /* since epoch */: Long = 0
        ...
        fun postExecute() {
            ...
            synchronized(BackgroundExecutor::class.java) {
                ...
                /* execution complete */TASKS.remove(this)
                if (serial != null) {
                    val next = take(serial!!)
                    if (next != null) {
                        if (next.remainingDelay != 0L) {
                            /* the delay may not have elapsed yet */
                            next.remainingDelay = Math.max(
                                0L,
                                targetTimeMillis - System.currentTimeMillis()
                            )
                        }
                        /* a task having the same serial was queued, execute it */execute(
                            next
                        )
                    }
                }
